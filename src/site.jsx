...
import { Building2, Droplets, ShieldCheck, FileSpreadsheet, HardHat, Workflow, Mail, Phone, MapPin, ExternalLink } from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <header className="sticky top-0 z-30 backdrop-blur bg-white/80 border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="#home" className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-2xl bg-blue-600 flex items-center justify-center shadow text-white font-bold">H</div>
            <div>
              <p className="text-sm tracking-wide text-gray-600">Higher Consulting Group, LLC</p>
              <h1 className="text-lg font-semibold leading-4">Engineering & Realty Services</h1>
            </div>
          </a>
          ...
          <a href="#contact" className="inline-flex items-center gap-2 rounded-2xl bg-blue-600 px-4 py-2 text-white shadow hover:bg-blue-700">
      ...
      <section id="home" className="relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 py-20 md:py-28 grid md:grid-cols-2 gap-10 items-center">
          <div>
            <span className="text-xs uppercase tracking-widest text-blue-700">Municipal & Public-Sector Advisory</span>
            <h2 className="mt-3 text-4xl md:text-5xl font-extrabold leading-tight">
              Practical expertise for <span className="text-blue-700">Safe, Clean Water</span> and resilient communities
            </h2>
            <p className="mt-4 text-gray-700 max-w-prose">
              Higher Consulting Group partners with counties and cities to deliver stormwater capture projects, SCWP feasibility studies, and capital delivery support—combining engineering discipline with clear processes and accountability.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#services" className="rounded-2xl bg-blue-600 text-white px-5 py-3 shadow hover:bg-blue-700">Explore services</a>
              <a href="#contact" className="rounded-2xl bg-white px-5 py-3 shadow border hover:bg-gray-50">Request a consultation</a>
            </div>
          </div>
          ...
          <div className="md:pl-6">
            <div className="grid grid-cols-2 gap-4">
              {[
                { icon: Droplets, title: "SCWP & Compliance", text: "Strategy, scoring, and funding roadmaps." },
                { icon: Building2, title: "Engineering & Realty", text: "Entitlements, permitting, stakeholder coordination." },
                { icon: ShieldCheck, title: "Code & Standards", text: "Program design, notices, hearings (BRAB)." },
                { icon: FileSpreadsheet, title: "Reporting & Funding", text: "Board updates, FEMA/PA documentation, dashboards." },
              ].map((c, i) => (
    ...
    <div className="mt-8 grid md:grid-cols-3 gap-6">
      ...
      <section id="approach" className="py-16 bg-white border-t">
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-8 items-start">
          <div className="md:col-span-1">
            ...
            <p className="mt-2 text-gray-700">Clear scope, structured execution, and measurable outcomes.</p>
          </div>
      ...
      <section id="clients" className="py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="mt-2 text-gray-700">Counties, cities, special districts, and public agencies across Southern California.</p>
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
            {["County Departments", "Public Works", "Water Agencies", "City Managers", "Code Enforcement", "Recovery Divisions", "Planning", "FEMA/PA Teams"].map((t, i) => (
              <div key={i} className="rounded-xl border bg-white p-4 text-center">{t}</div>
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-10">
          <div>
            ...
            <div className="mt-6 space-y-3 text-gray-700">
              <p className="flex items-center gap-2"><Mail className="w-4 h-4"/> hello@higherconsultinggroup.com</p>
              <p className="flex items-center gap-2"><Phone className="w-4 h-4"/> (###) ###-####</p>
              <p className="flex items-center gap-2"><MapPin className="w-4 h-4"/> Southern California • Available statewide</p>
            </div>
          </div>

          <form onSubmit={(e)=>{ e.preventDefault(); alert('Demo form — connect to a real handler later.'); }} className="rounded-2xl bg-white p-6 border shadow-sm">
            <div className="grid gap-4">
              <div>
                <label className="text-sm">Name</label>
                <input className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="Full name" required />
              </div>
              <div>
                <label className="text-sm">Email</label>
                <input type="email" className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="you@example.gov" required />
              </div>
              <div>
                <label className="text-sm">Organization</label>
                <input className="mt-1 w-full rounded-xl border px-3 py-2" placeholder="Agency / Department" />
              </div>
              <div>
                <label className="text-sm">Message</label>
                <textarea className="mt-1 w-full rounded-xl border px-3 py-2" rows={4} placeholder="Briefly describe goals or project" required />
              </div>
              <button type="submit" className="rounded-2xl bg-blue-600 text-white px-5 py-3 shadow hover:bg-blue-700 inline-flex items-center gap-2">
                <Mail className="w-4 h-4"/> Send
              </button>
              <p className="text-xs text-gray-500">By submitting, you consent to be contacted about this inquiry. No marketing spam.</p>
            </div>
          </form>
        </div>
      </section>
      ...
      <footer className="border-t bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 py-8 text-sm text-gray-600 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <a href="#" className="inline-flex items-center gap-1 hover:text-blue-700"><ExternalLink className="w-3.5 h-3.5"/> Capability Statement (PDF)</a>
